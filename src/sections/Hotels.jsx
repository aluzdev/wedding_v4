      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        <ul className="reveal mt-10 grid gap-4 sm:grid-cols-2">
          {config.hotels.map((hotel, i) => {
            ...
            const cardClass =
              'flex h-full gap-4 rounded-2xl border border-ink/10 bg-cream-soft px-5 py-5'
            return (
              <li key={i}>
                {hotel.url ? (
                  <a
                    ...
                    className={`${cardClass} transition-colors hover:border-moss/40 hover:bg-cream/70`}
                  >
                    {body}
                  </a>
                ) : (
